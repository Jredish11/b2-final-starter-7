...
RSpec.describe "invoices show" do
  before :each do
    ...
    @active_coupon2 = Coupon.create!( coupon_name: "12off", coupon_code: "TensdfsdaER", merchant_id: @merchant1.id, status: 1, discount_amount: 0.1, discount_type: 0) 
    ...
    @invoice_2 = Invoice.create!(customer_id: @customer_1.id, status: 2, created_at: "2012-03-28 14:54:09", coupon_id: @active_coupon2.id)
    ...
    @ii_10 = InvoiceItem.create!(invoice_id: @invoice_2.id, item_id: @item_5.id, quantity: 1, unit_price: 1, status: 1)
...
#   7. Merchant Invoice Show Page: Subtotal and Grand Total Revenues
  ...
  it "displays the subtotal for the merchant from this invoice" do 
    visit merchant_invoice_path(@merchant1, @invoice_1)
    within "#subtotal" do
  ...
  it "displays the grand total revenue after the discout was applied" do 
    visit merchant_invoice_path(@merchant1, @invoice_1)
    within "#subtotal" do
      expect(@invoice_1.grand_total_revenue).to eq(157.0)
    end
  end
